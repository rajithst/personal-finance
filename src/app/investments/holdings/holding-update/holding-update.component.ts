...
export class HoldingUpdateComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    this.transactionForm.get('company')?.valueChanges.subscribe((value) => {
      if (value) {
        const selectedCompany = this.companies().find((x) => x.symbol === value);
        if (selectedCompany) {
          this.company.set(selectedCompany);
          this.transactionForm.get('stock_currency')?.setValue(selectedCompany.stock_currency)
        }
      }
    });
  }
  ...
  private getNewTransactionForm() {
    return new FormGroup({
      ...
      purchase_price: new FormControl<number | null>(null, [Validators.required]),
      settlement_currency: new FormControl<string | null>(null, [Validators.required]),
  ...
  submit() {
    ...
    this.transactionForm.value.quantity = Number(this.transactionForm.value.quantity);
    this.transactionForm.value.purchase_price = Number(this.transactionForm.value.purchase_price);
